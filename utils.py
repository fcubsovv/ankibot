...
if not GROQ_KEY:
    logger.error("Критическая ошибка: GROQ_API_KEY не задан в .env")
...
def generate_card(word: str) -> str:
    logger.info(f"Отправка запроса в Groq Llama для слова: '{word}'")
...
def parse_card(card_text: str) -> dict:
    ...
    # Добавили explanation в словарь
    result = {"front": "", "synonyms": "", "sentence": "", "translation": "", "explanation": "", "image_prompt": ""}
    for line in lines:
...
async def ankiweb_add_card(card_text: str, audio_url: str) -> bool:
    ...
    # 3. Генерируем красивый HTML для FRONT
    front_html = f"""
    ...
    # 4. Генерируем красивый HTML для BACK
    synonyms_block = f'''
